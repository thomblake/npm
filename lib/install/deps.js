...
function fixRequires (tree, child, name) {
  //assume that child is required by tree, fix it for all direct children of root.
  var root = tree
  while (root.parent !== null) {
    root = root.parent
  }
  root.children.forEach(function (rootChild) {
    if (moduleName(rootChild) === moduleName(child)) {
      replaceModuleName(rootChild.package, '_requiredBy', name)
      replaceModule(rootChild, 'requiredBy', tree)
    }
  })
}
